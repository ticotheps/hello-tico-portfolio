module.exports = {
  /*------------------Global Color Variables-----------------*/
  /* links in the top navBar */
  background: "#FFFFFF" /* white */,
  /* middle triangle in "Landing" section */
  backgroundDark: "#c4fffd" /* light cyan */,
  /* title text in "Landing" section */
  primary: "#3257fa" /* royal blue */,
  /* used to underline text (i.e. - section headers) */
  primaryLight: "#7c00e7" /* dark violet */,
  /* top-right triangle in "Landing" section + sticky navBar */
  primaryDark: "#3257fa" /* royal blue */,
  /* top-left triangle in "About Me" section */
  secondary: "#05d1ff" /* deep sky blue */,
  /* stripe between "About Me" & "Projects" section */
  secondaryLight: "#05d1ff" /* deep sky blue */,
  /* top-right triangle in "Landing" section + sticky navBar */
  secondaryDark: "#3257fa" /* royal blue */,
  /*------------------Components-----------------*/
  /* Card.js */
  card_bg: "#FFFFFF" /* white */,
  onHover_card_bg: "#20ffb9" /* medium spring green */,
  /* Footer.js */
  footer_text: "#FFFFFF" /* white */,
  footer_bg: "#3257fa" /* royal blue */,
  footer_social_links: "#FFFFFF" /* white */,
  /* Header.js */
  disappearing_navBar_bg: "#ff6d6d" /* salmon */,
  /* LinkAnimated.js */
  section_heading_underline: "#3257fa" /* royal blue */,
  onHover_section_heading_underline: "#20ffb9" /* medium spring green */,
  /* MarkdownRenderer.js */
  onHover_aboutMe_text_bg: "#3257fa" /* royal blue */,
  /* MouseIcon.js */
  mouse_icon_border: "#3257fa" /* royal blue */,
  mouse_icon_dot: "#3257fa" /* royal blue */,
  /* Section.js */
  section_heading: "#7c00e7" /* dark violet */,
  /* SocialLink.js */
  social_link: "#3257fa" /* royal blue */,
  onHover_social_link: "#7c00e7" /* dark violet */,
  onHover_underline_social_link: "#20ffb9" /* medium spring green */,
  /* About.js */
  profile_pic_border: "#7c00e7" /* dark violet */
};
